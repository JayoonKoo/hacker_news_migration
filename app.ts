...
abstract class View {
  ...
  protected setTmeplateData(key: string, value: string) {
    this.renderTemplate = this.renderTemplate.replace(`{{__${key}__}}`, value);
  }

  private cleartHtmlList(): void {
    this.htmlList = [];
  }
...
class Router {
  ...
  constructor() {
    window.addEventListener("hashchange", this.route.bind(this)); // 이렇게 하면 this 가 route 에 인스턴스가 아니게 된다.
...
class NewsFeedView extends View {
  ...
  render(): void {
    ...
    const under =
      store.currentPage * 10 > this.maxLength
        ? this.maxLength
    ...
    this.setTmeplateData(
      "next_page",
      String(
        store.currentPage < this.maxIndex
          ? store.currentPage + 1
          : this.maxIndex
      )
    );
...
class NewsDtailView extends View {
  constructor(containerId: string) {
    let template = `
  ...
  render() {
    ...
    this.setTmeplateData("currentPage", String(store.currentPage));
